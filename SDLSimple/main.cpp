...
// Define the position for broder
...
/* Sources:
 Pink Racket: https://images.app.goo.gl/yHAahAkAAnLHnP9Y6
 Red Racket: https://images.app.goo.gl/54V7fpWbdKoofVVd8
 Tennis Court: https://images.app.goo.gl/nXXCucZstAMK8Vk29
 Tennis Ball:
*/
// File Path:
...
constexpr char BALL_ONE_SPRITE_FILEPATH[] = "ball_1.png";
constexpr char BALL_TWO_SPRITE_FILEPATH[] = "ball_2.png";
constexpr char BALL_THREE_SPRITE_FILEPATH[] = "ball_3.png";

// ID
GLuint g_tennis_court_id, g_left_racket_id, g_right_racket_id, g_tennis_ball_id_1, g_tennis_ball_id_2, g_tennis_ball_id_3, g_left_win_id, g_right_win_id, g_ball_set_1_id, g_ball_set_2_id, g_ball_set_3_id;
...
// Define the object matrices for all the objects.
glm::mat4 g_view_matrix,
          ...
          g_model_matrix_ball_set_1,
          g_model_matrix_ball_set_2,
          g_model_matrix_ball_set_3,
          g_projection_matrix;
...
// We want filp the pink racket initially
...
constexpr glm::vec3 BALL_SETTING_INIT_POS = glm::vec3(-2.0f, -3.5f, 0.0f);

// Global variables that control the game's status
...
// Use for different amount of balls control
float ballnumber = 1;
...
constexpr glm::vec3 BALL_SETTING_INIT_SCALE = glm::vec3(0.7f, 0.8f, 0.0f);
...
void initialise()
{
    ...
    g_model_matrix_ball_set_1 = glm::mat4(1.0f);
    g_model_matrix_ball_set_2 = glm::mat4(1.0f);
    g_model_matrix_ball_set_3 = glm::mat4(1.0f);
    ...
    g_ball_set_1_id = load_texture(BALL_ONE_SPRITE_FILEPATH);
    g_ball_set_2_id = load_texture(BALL_TWO_SPRITE_FILEPATH);
    g_ball_set_3_id = load_texture(BALL_THREE_SPRITE_FILEPATH);
    glEnable(GL_BLEND);
...
void process_input()
{
    ...
    while (SDL_PollEvent(&event))
    {
        switch (event.type)
        {
            ...
            case SDL_KEYDOWN:
                    ...
                    {
                        // Switch to between single player mode and two players mode when we presses t button
                        case SDLK_t:
                            singlePlayerModeOpened = !singlePlayerModeOpened;
                            break;
                        // Control the number of balls by clicking 1, 2, 3
                        case SDLK_1:
                            if(!ifGameStop){
                                ballnumber = 1;
                            }
    ...
    // Racket's movement by users
    if (key_state[SDL_SCANCODE_W] && !ifGameStop)
    {
        // Make sure the rackets can not go beyond the broder
...
void update()
{
    ...
    // Change all the objects' new position in this frame accroding
    ...
    // Model matrix reset and Apply transformations
    g_model_matrix_left_paddle = glm::mat4(1.0f);
    ...
    g_model_matrix_ball_set_2 = glm::mat4(1.0f);
    g_model_matrix_ball_set_3 = glm::mat4(1.0f);
    g_model_matrix_ball_set_1 = glm::translate(g_model_matrix_ball_set_1, BALL_SETTING_INIT_POS);
    g_model_matrix_ball_set_2 = glm::translate(g_model_matrix_ball_set_2, BALL_SETTING_INIT_POS);
    g_model_matrix_ball_set_3 = glm::translate(g_model_matrix_ball_set_3, BALL_SETTING_INIT_POS);
    g_model_matrix_ball_set_1 = glm::scale(g_model_matrix_ball_set_1, BALL_SETTING_INIT_SCALE);
    g_model_matrix_ball_set_2 = glm::scale(g_model_matrix_ball_set_2, BALL_SETTING_INIT_SCALE);
    g_model_matrix_ball_set_3 = glm::scale(g_model_matrix_ball_set_3, BALL_SETTING_INIT_SCALE);
    
    // Here ifLeftWin will be act as a selector (true/false), that when the game is stopped, then we select either Pink win or Red win by the selector's value
    ...
    // Update number of balls accroding to the current ballnumber value in this frame
    ...
    // Detect box-to-box collison for bounce off, one for each ball
    ...
    // The logic here is that we want to rotate both x force vector and y force vector 90 degree to implement the bounce off for both broder and "paddles".
    ...
    // Game Over, when any of the ball hit the left or right broder, set the movement for all the balls to 0 for the next frame, so that the game will be stopped.
    ...
    if(singlePlayerModeOpened){
        if(g_model_matrix_right_paddle_position.y >= TOP_BORDER){
            // we only change the moving vector's direction when the racket either touched up broder or down broder.
...
void render() {
    ...
    // Render number of balls accroding to the ballnumber value in each frame
    if(ballnumber == 1){
        draw_object(g_model_matrix_ball_1, g_tennis_ball_id_1);
        draw_object(g_model_matrix_ball_set_1, g_ball_set_1_id);
    }else if(ballnumber == 2){
        draw_object(g_model_matrix_ball_1, g_tennis_ball_id_1);
        draw_object(g_model_matrix_ball_2, g_tennis_ball_id_2);
        draw_object(g_model_matrix_ball_set_2, g_ball_set_2_id);
    } else if(ballnumber == 3){
        draw_object(g_model_matrix_ball_1, g_tennis_ball_id_1);
        draw_object(g_model_matrix_ball_2, g_tennis_ball_id_2);
        draw_object(g_model_matrix_ball_3, g_tennis_ball_id_3);
        draw_object(g_model_matrix_ball_set_3, g_ball_set_3_id);
    }
    
    // Render the two rackets
    draw_object(g_model_matrix_left_paddle, g_left_racket_id);
    draw_object(g_model_matrix_right_paddle, g_right_racket_id);
    
    // Render the winning message only if the game has been stopped
    if(ifGameStop && ifLeftWin){
